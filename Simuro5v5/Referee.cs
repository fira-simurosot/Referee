...
namespace Referee.Simuro5v5
{
    public class Referee : ICloneable
    {
            ...
            if (judgeResult.WhoisFirst == target)
            {
                ...
                for (int i = 0; i < Const.RobotsPerTeam; i++)
                {
                        ...
                        || !PlaceOffensivePosSquare[i].square.IsOverlapWithRectangle(stadiumState))
                    {
                        ...
                        JudgeSafePosSquare(PlaceDefenderSafePosSquare, PlaceOffensivePosSquare, PlaceDefenderPosSquare);
                        JudgeSafePosSquare(PlaceOffensiveSafePosSquare, PlaceOffensivePosSquare,
                            PlaceDefenderPosSquare);
                    }
                }
            ...
            if (judgeResult.WhoisFirst == target)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    if (GoalieOffensivePosSquare[GoalieId].square.ContainsPoint(BallPos))
                    {
                        ...
                        JudgeSafePosSquare(GoalieOffensiveSafePosSquare, GoalieOffensivePosSquare,
                            GoalieDefenderPosSquare);
                        JudgeSafePosSquare(GoalieDefenderSafePosSquare, GoalieOffensivePosSquare,
                            GoalieDefenderPosSquare);
                    }
                    else
                        break;
        ...
        /// </summary>
        private bool RobotCrossByRobots(RobotPosSquare robotPosSquare, RobotPosSquare[] robotsPosSquare,
            bool self = false,
            int selfid = -1)
        {
            //默认是检测对面组别
